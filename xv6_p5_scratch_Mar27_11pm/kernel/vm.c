...
{
  uint faultAddr = rcr2();
    if(faultAddr > USERTOP){
      proc->killed = 1;
      cprintf("CoW: Invalid virtual address\n");
    }else{
        //get refer count
      uint pa = PADDR(faultAddr);
      if(getRef(pa) == 1){
        ...
        //alloc a new page so we can copy original page
        char *mem = kalloc();
        if ((mem = kalloc()) == 0){
          cprintf("error kalloc in pagefault, breaking.");
        } else{
          memmove(mem, (char *)pa, PGSIZE);
          if(minusRef(pa) == -1){  //decrement refer count of original page
            cprintf("error in kalloc in pagefault, breaking");
          }else{
            //flush tlb
            lcr3(PADDR(proc->pgdir));
            //replace entry pgtable with the writable copy, use uva2ka to get PTE 
            pte_t* pte = (pte_t *) uva2ka(proc->pgdir, (char *) &faultAddr); 
            if(*pte == 0){
              cprintf("uva2ka return 0, breaking");
            }else{
              *pte = (pte_t ) mem;
              *pte = *pte | (PTE_W) | PTE_U;
            }
        ...
        proc->killed = 1;
        cprintf("ref count of page <= 0, killing and exit\n");    
      }
